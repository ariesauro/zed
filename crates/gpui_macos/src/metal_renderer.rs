...
pub(crate) struct MetalRenderer {
    ...
    // Torus: single-plane BGRA surfaces (RGB viewport), alongside the YUV path.
...
impl MetalRenderer {
    ...
    fn new_internal(
        ...
        // Torus: same vertex shader, single-texture BGRA fragment (no YUV convert).
    ...
    fn draw_surfaces(
        ...
        for surface in surfaces {
            ...
            // Two supported layouts: biplanar YUV 4:2:0 (video) and single-plane
            // BGRA (Torus RGB viewport). Pick the matching pipeline + texture binding.
            let pixel_format = surface.image_buffer.get_pixel_format();
            let is_bgra = match pixel_format {
                kCVPixelFormatType_420YpCbCr8BiPlanarFullRange => false,
                kCVPixelFormatType_32BGRA => true,
                other => panic!("unsupported surface pixel format {other:#010x}"),
            };
            ...
            if is_bgra {
                ...
                // Single BGRA8Unorm plane sampled straight through (no YUV math);
                // bytes are passed through to the BGRA8Unorm drawable like the img() path.
